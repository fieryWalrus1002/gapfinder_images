...
def get_processed_image(image_path, masks_path, process_name):
    ...
    return cv2.imread(
        f"{masks_path}/{process_name}/{os.path.basename(image_path)}",
...
def plot_images_for_process(process_name, masks_path):
    ...
    for image_name in images:
        image, mask = load_images_for_given_process(
            image_name, masks_path, process_name
        )
...
def execute_process(process_name, masks_path, output_path, images):
    ...
    hashes = "#" * num_hash
    process_name_hashes = "#" * len(process_name)
    print(f"{hashes}#{process_name_hashes}#{hashes}")
    ...
    for i, image_name in enumerate(images):
        ...
        lumen_image_filename = (
            f"{output_directory}/lumen/{os.path.basename(image_name)}"
        )
        membrane_image_filename = (
            f"{output_directory}/membrane/{os.path.basename(image_name)}"
        )
        ...
        filtered_image_name = f"{output_directory}/contour_comparison/{os.path.basename(image_name).replace('.png', '')}_{process_name}_filtered.png"
