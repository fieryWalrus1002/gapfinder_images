...
if __name__ == "__main__":
    ...
    ####################### JUST otsu then offset thresholding ##############################
    ...
    thresh_levels = [90, 100, 130] # the threshold value to use
    ...
    # get the existing x_otsu thresholding metadata names from output\trial_1\processed_images\
    # 19_ostuOffset
    # existing_otsu_offset_keys = [key for key in os.listdir(f"{base_path}/processed_images/") if "ostuOffset" in key]

    # get the max value from the existing keys
    # max_key = max([int(key.split("_")[0]) for key in existing_otsu_offset_keys])
    # we did it once, if we do it again we'll have to change the max_key
    max_key = 99
    ...
    for i, (th_key, morph_key, equalize, blur, thresh, maxval, blocksize, c, morph_arg, offset) in enumerate(otsu_offset_combinations):
        ...
        if th_dict is not None:
            th_dict["key"] = f"{i0}_{th_key}"
            otsu_offset_th_metadata[th_dict["key"]] = th_dict
        else:
            
            print(f"Skipping {i0}_{th_key}")
    ...
    all_th_metadata_df.to_csv(f"{base_path}/threshold_metadata_{run_number}.csv", index=True)
