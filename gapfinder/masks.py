...
def get_isolated_membrane(
    image, method, blur=True, equalize=True, metadata=None, th_metadata=None
):
    ...
    processed = process_image(
        ...
        equalize=equalize,
        metadata=metadata,
        th_metadata=th_metadata[method],
    )
...
def add_membrane_overlay(image, overlay, color=(0, 0, 0)):
    ...
    result = cv2.bitwise_and(image_bgr, mask_inv_bgr) + cv2.bitwise_and(
        color_image, cv2.cvtColor(overlay, cv2.COLOR_GRAY2BGR)
    )
...
def get_filename(base_path, suptitle, strip: str):
    ...
    output_filename = (
        f"{output_dir}/{suptitle.replace(' ', '_').replace(',', '')}_strip_{strip}.png"
    )
...
def plot_images(base_path: str, image_dict: dict, titles=None, display_image=True):
    ...
    images = [
        image_dict["image"],
        image_dict["thresholded"],
        image_dict["morphed"],
        image_dict["overlayed"],
    ]
    ...
    cv2.imwrite(
        f"{mask_output_path}/strip_{image_dict['metadata']['strip'].values[0]}.png",
        image_dict["morphed"],
    )
...
def adaptive_threshold(image, options: dict):
    ...
    thresholded = cv2.adaptiveThreshold(
        image, maxval, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, blocksize, c
    )
    return thresholded
...
def adaptive_gaussian_threshold(image, options: dict):
    ...
    c = options.get("c", 2)
    thresholded = cv2.adaptiveThreshold(
        image, maxval, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, blocksize, c
    )
    return thresholded
...
def morph_close_then_open(image, morph_args: dict):
    ...
    """
    if (
        (not "closing_kernel" in morph_args)
        or (not "opening_kernel" in morph_args)
        or (not "closing_iterations" in morph_args)
        or (not "opening_iterations" in morph_args)
    ):
        raise ValueError(
            "Morphological arguments must include closing_kernel, opening_kernel, closing_iterations, and opening_iterations"
        )

    closing1 = cv2.morphologyEx(
        image,
        cv2.MORPH_CLOSE,
        morph_args["closing_kernel"],
        iterations=morph_args["closing_iterations"],
    )
    opening2 = cv2.morphologyEx(
        closing1,
        cv2.MORPH_OPEN,
        morph_args["opening_kernel"],
        iterations=morph_args["opening_iterations"],
    )
    return opening2
...
def create_threshold_metadata(
    base_path: str, image_dir: str, roi_metadata_path: str, options: dict
):
    ...
    threshold_functions = {
        "otsuOffset": otsu_then_offset_threshold,
        "otsu": otsu_threshold,
    ...
    otsu_offset_combinations = product(
        options["th_method_keys"],
        options["morph_keys"],
        options["opts_equalize"],
        options["opts_blur"],
        options["thresh_levels"],
        options["maxval_levels"],
        options["blocksize_levels"],
        options["c_levels"],
        options["morph_args"],
        options["offset_values"],
    )
    ...
    for i, (
        ...
        morph_arg,
        offset,
    ) in enumerate(otsu_offset_combinations):

        th_dict = create_th_dict(
            th_key,
            morph_key,
            equalize,
            blur,
            ...
            blocksize,
            c,
            threshold_functions[th_key],
            morph_filter_functions[morph_key],
            morph_arg,
            offset,
        )
    ...
    print(
        f"Created {len(otsu_offset_th_metadata)} otsu offset thresholding metadata dictionaries"
    )
    ...
    all_th_metadata_df = all_th_metadata_df.drop(
        columns=["threshold_func", "morph_filter_func"]
    )
...
def create_masks(
    base_path: str, image_dir: str, roi_metadata_path: str, all_th_metadata: dict
):
    ...
    for key, th_metadata in all_th_metadata.items():
        ...
        for strip_num in roi_metadata["strip"].unique():
            strip_filename = roi_metadata[roi_metadata["strip"] == strip_num][
                "strip_filename"
            ].values[0]
            base_filename = os.path.basename(strip_filename.strip(".png"))
            ...
            plot_images(
                base_path=base_path,
                image_dict=process_image(image, key, strip_metadata, th_metadata),
                titles=titles,
                display_image=False,
            )
